...
class RequestRetry private constructor() : INetworkListener {
    ...
    override fun onNetworkState(state: Boolean) {
        when (state) {
            false -> {
                LogUtil.ii("retry", "网络已断开")
                stop()
            }
            else -> {
                LogUtil.ii("retry", "网络已连接")
                start()
            }
        }
    }

    fun getQueueList(): List<Any> {
        return if (queue.toArray() == null) {
            mutableListOf()
        } else {
    ...
    fun registerNetworkReceiver(context: Context): RequestRetry {
        ...
        LogUtil.ii("retry", "当前状态网络状态: ${!isTerminate}")
    ...
    fun <T> addRequest(t: T, method: String = ""): Boolean {
        ...
        if (retryHashMap.containsKey(key)) {
            ...
            return if (!mRecordMap.containsKey(retryBean)) {
                ...
                LogUtil.ii("retry", "添加retryBean到队列:${retryBean}")
                true
            } else {
                LogUtil.ii("retry", "队列已存在记录: ${retryBean}不添加")
                false
            }

        } else {
            LogUtil.ii("retry", "未找到对应的方法")
            LogUtil.ii("retry", "上传类找到的信息:${retryHashMap}")
        }
        return false
    }
        ...
        return when (val retryBean = queue.poll()) {
            is RetryBean<*> -> {
                if (mRecordMap.containsKey(retryBean)) {
                    ...
                    LogUtil.ii("retry", "数据未存在插入信息队列中或取出在插入之前:$retryBean")
                    retryBean
                }
            }
            else -> {
