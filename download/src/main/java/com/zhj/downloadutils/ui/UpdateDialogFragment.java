...
//        getDialog().getWindow().addFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL);
        ...
        lp.height = (int) (displayMetrics.heightPixels);
    ...
    private void initData() {
        mUpdateApp = (UpdateAppBean) getArguments().getSerializable(Constants.INTENT_KEY);
        //设置主题色
        initTheme();
        if (mUpdateApp != null) {
            //弹出对话框
    ...
    private void initTheme() {
        final int color = getArguments().getInt("", -1);
        final int topResId = getArguments().getInt("", -1);
        if (-1 == topResId) {
            if (-1 == color) {
                //默认红色
...
//            }
    ...
    private void startDownloadApp(DownloadService.DownloadBinder binder) {
        ...
        if (mUpdateApp != null) {
            ...
            binder.start(mUpdateApp, new DownloadService.ProgressCallback() {
                @Override
                public void onStart() {
                ...
                public void onProgress(float progress, long totalSize) {
                    if (!UpdateDialogFragment.this.isRemoving() && getActivity() != null) {
