    ...
    public static boolean installApp(Activity activity, File appFile) {
        if (Build.VERSION.SDK_INT >= 26) {
            ...
            if (b) {
                ...
                //请求安装未知应用来源的权限
                ActivityCompat.requestPermissions(activity, new String[]{Manifest.permission.REQUEST_INSTALL_PACKAGES}, REQ_CODE_INSTALL_APP);
            }
        } else {
            installApp(activity.getApplicationContext(), appFile);
        }
        return false;
    }

    public static boolean installApp(Fragment fragment, File appFile) {
        EventBus.getDefault().post(new InstallEvent(appFile));
        return true;
    }
    ...
    public static Intent getInstallAppIntent(Context context, File appFile) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                Uri fileUri = FileProvider.getUriForFile(context, "com.zhj.downloadutils.app.fileprovider", appFile);
